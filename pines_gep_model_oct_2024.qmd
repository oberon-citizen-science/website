...
In the case of the proposed Pines Wind Farm, the developers are offering to visit the residences and properties of interested community members to allow them to visualise the proposed Pines wind farm using augmented reality (AR) software on a tablet computer -- presumably this combines simulations of the wind turbines with a photo of the view from specific viewpoints at the community member's home or property (at the time of writing, this is unverified, but we will update this description of the Pines AR visualiation facility when more details are known). 

Although such personalised use of AR software is commendable, the capacity of the Pines developer team to visit the homes or properties of every interested community member is obviously finite, and at this stage, technical details and documentation of the  accuracy and validity of such AR visualisations has not been made available by the Pines developers. For this reason, OCSN has prepared the visualisations provided here. These may be less sophisticated than the AR visualisations offered by the Pines developers, but they are accessible to anyone with access to a laptop or desktop computer, using entirely free software, and the methodology used in their preparation is open, fully documented and able to be reproduced by anyone who wishes to.
...
As of mid-October 2024, the Pines developers have released only limited information about the proposed layout and details of the wind farm. What we know is gleened from this [press release](assets/Pines_Press_Release_Draft_Turbine_Layout_10_Oct_2024.pdf) and from [this map](https://api.mapbox.com/styles/v1/klaraahlin/cm29d1yj300k401pi0i6ncf4p.html?access_token=pk.eyJ1Ijoia2xhcmFhaGxpbiIsImEiOiJOOUlGSVI0In0.68r4s52l711tWGnj0kwUsA&#11/-33.967/149.6921/0/13) on the Pines Wind Farm web site (static archived screenshot available [here](assets/Pines_layout_10_Oct_2024.png)). From these we know the proposed locations of the "approximately 250" turbines (242 actual locations are shown on the map) and that the entire wind farm will have a nominal generating capacity of 2 gigawatts (2 GW or 2,000 MW). That means that the nominal or "nameplate" capacity of each turbine will be around 8 megawatts (8 MW) (2000 MW &divide; 250 = 8 MW). [Elsewhere on the Pines web site](https://www.thepineswindfarm.com.au/about-the-project) it states that the turbines will be up to 300 metres tall. All currently available 8 MW terrestrial wind turbines are well under 300 m tall at the uppermost tip of the blade arc. Therefore we have chosen to include two types of 3D model. 

The first uses the dimensions for a reference 8 MW wind turbine published in [this scientific paper](https://iopscience.iop.org/article/10.1088/1742-6596/753/9/092013/pdf) -- the specific dimensions are a tower height of 110 m, a tower base diameter of 7.7 m, and a tower top diameter of 5 m, nacelle diminsions of 20 m x 7.5 m x 7.5 m, a rotor diameter of 164 m, a blade root length of 2.5 m and diameter 2 m, a blade chord at the root of 5.4 m with a 50% taper to the tip, and a blade twist of 30 degrees over its length. This turbine is thus 192 m high at the blade tip at the top of its arc. 

The second 3D model uses the same dimensions for the turbine nacelle and blades but makes the tower 212.5 m tall, to give an overall height of 300 m to the blade tip at the top of its arc. The tower diameter at the base was increased using engineering principles to account for the additional stresses experienced by such a tall tower. OCSN acknowledges that such a turbine is very unlikely to be used, but it has been included to illustrate the impact on visibility of turbines at the maximum height allowed for the proposed Pines Wind Farm under its planning permit. It is more likely that the developers would use larger but fewer turbines if a 2 GW total capacity is retained. Currently available 12 or 14 MW turbines are between 260 m and 280 m high. However, we do not know and cannot guess where the developers might chose to locate a smaller number of larger turbines, so we have stuck to showing 242 turbines, either 192 m high or 300 m high, at this stage. These models can and will be updated if and when the Pines developers release further information about the proposed turbine types and dimensions.

For each of these two 3D turbine models, we have provided four different sets of turbines models. Wind turbines use a yaw motor to rotate them to face the wind, and thus they may differ in appearance from any given location depending on the wind direction. To model this, we have given each of the 242 turbines a random orientation but generally facing a prevailing wind direction: West (W), South-west (SW), North-west (NW) or East (E). 
...
::: {.aside}
There are alternate ways to get to _Ground View_ mode. You could simply click near the Placemark; keep clicking but not too fast, until you are at Ground View. Or you could hold the cursor/crosshairs over the location you want for a viewpoint and use the mouse scroll-wheel to get down to ground-level.
:::
...
Now you can always get back to that view. Click the _Exit ground-level view_ button in the upper right corner of the window (if it is not visible, just move around the view a bit). The Placemark will become visible from an elevated view. Either double-click on the Placemark in _My Places_, or double-click at the base of the Placemark to retrieve the view.
...
![Home view in Google Earth Pro](assets/gep_home_view.png)
...
The **Tool Bar** at the tope of the GEP window includes many useful tools, such as a Ruler for measurements, Sun to adjust the direction of sunlight, or creating features such as a Placemark, Polygon or Path. Placing the mouse cursor over a tool reveals its purpose.
...
* **Pegman**. Dragging Pegman on to the map changes the screen to Street View (and/or Terrain View). Click and drag Pegman onto the map and the screen changes to Terrain View (make sure that Terrain is checked in the Layers panel). When Pegman is placed on a blue road, the screen changes to Street View. 
* **Zoom**. This is the vertical slider control below Pegman. Moves the view closer or further away.

### Navigation tools for Ground or Street View modes

*	**Ground View**. This is the small box in the upper-right corner with a little Pegman icon and a little building icon. It allows you to select Pegman’s Street View (if available) or Terrain View, and the button to its right labelled _Exit ground-level view_ exits back to Earth View.
