...
The calculated viewsheds are for one of the proposed Palings Yard wind farm towers at 240m height.
...
leaflet(height="800px", width="1200px") %>% 
  ...
  addRasterImage(terra_raster_no_correction_20m, 
                 ...
                 colors = terra_no_correction_pal,
                  group = "Proposed 240m turbine viewshed (no curvature)",
                  maxBytes = 4 * 8330 * 8801) %>%
  addRasterImage(terra_raster_20m, 
                 opacity=0.6, 
                 colors = terra_pal,
                  group = "Proposed 240m turbine viewshed (curvature)",
                  maxBytes = 4 * 8330 * 8801) %>%
  addMarkers(lng=palings_yard_turbines_untransformed_df[20,1],
             lat=palings_yard_turbines_untransformed_df[20,2],  
             label = "Proposed 240m turbine",
              group = "Proposed 240m turbine",
  ...
  addLayersControl(baseGroups = c( "Imagery", "Shaded relief", "Topography", "OpenStreetMap"),
                   overlayGroups = c( "Proposed 240m turbine",
                                      "Proposed 240m turbine viewshed (no curvature)",
                                      "Proposed 240m turbine viewshed (curvature)"),
...
This is the same viewshed as above (with curvature and refraction corrections enabled), but shaded by distance from the tower to de-emphasise distant viewpoints. This is a first step towards representing visual impact. 
...
leaflet(height="800px", width="1200px") %>% 
  ...
  addMarkers(lng=palings_yard_turbines_untransformed_df[20,1],
             ...
             label = "Proposed 240m turbine",
              ...
              labelOptions = labelOptions(noHide = TRUE, textsize = "10px")) %>%  
  addLayersControl(baseGroups = c("Imagery", "Shaded relief", "Topography", "OpenStreetMap"),
                   overlayGroups = c( "Proposed 240m turbine viewshed (curvature)"),
                   options = layersControlOptions(collapsed = FALSE)
  ) %>%
  addMeasure(
            position = "bottomleft",
