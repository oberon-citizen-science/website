---
title: "Visual Impact Assessment capability development"
---
...
terra_raster_no_correction_10m <- terra::aggregate(terra_raster_no_correction, fact=2, fun=max)
...
terra_raster_10m <- terra::aggregate(terra_raster, fact=2, fun=max)
...
This is the same viewshed as above (with curvature and refraction corrections), but shaded by distance from the tower to de-emphasise distance viewpoints. This is a first step towards representing visual impact. 
...
cobar_tower <- vect(cbind(st_coordinates(demo_turbine_sf)), crs=crs(terra_raster_10m))
# cobar_tower <- project(cobar_tower, terra_raster_10m)
tower_dist <- distance(terra_raster_10m, cobar_tower)

terra_raster_10m_dist <- terra_raster_10m * tower_dist

dist_pal <- colorNumeric(palette = "Reds", domain = minmax(terra_raster_10m_dist), reverse = TRUE)
...
leaflet(height="800px", width="1200px") %>% 
  ...
  addTiles(group = "OpenStreetMap") %>%
  addProviderTiles("Esri.WorldImagery", group="Imagery") %>%
  addRasterImage(terra_raster_10m_dist, 
                 opacity=0.6, 
                  colors = dist_pal,
                  group = "Putative 10m tower viewshed (curvature)",
                  maxBytes = 4 * 8330 * 8801) %>%
  addLegend(
    ...
    values = minmax(terra_raster_10m_dist),
...
binned_dist_pal <- colorBin(palette = "Reds", 
                            domain = minmax(terra_raster_10m_dist),
                            bins = c(800, 3200, 8100, 16100, 32200),
                            reverse = TRUE)
