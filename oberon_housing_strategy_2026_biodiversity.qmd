...
bionet_oberon <- read_tsv("assets/Atlas_records_20260613-134825/Atlas_records_gsodezosieotdxr2qjvoceio20260613-134818.txt", skip=4) |>
  separate_wider_delim(ScientificName, delim = " ", names = c("genus_name", "species_name"),
                       too_many = "drop")

bionet_greater_gliders_oberon <- bionet_oberon |>
  filter(genus_name == "Petauroides", species_name == "volans") |>
  separate_wider_delim(DateFirst, delim = " ", names = c("DateFirst_date", "DateFirst_time"), too_few = "align_start") |>
  mutate(DateFirst_date = lubridate::dmy(DateFirst_date))
...
bionet_oberon_filtered_sf <- bionet_oberon |> 
                        ...
                        mutate(species_statuses = 
                                 paste("Aust:", Comm_status_condensed,
                                       "; NSW:", NSW_status_condensed,
...
bionet_oberon_filtered_sf %>%
  filter(year(DateLast) > 1990, !Comm_status_condensed == "Nil") %>%
  ggplot(aes(x=year(DateLast), fill=Comm_status_condensed)) +
           geom_bar(position = position_dodge2(preserve = "single")) +
  theme_minimal() +
  theme(legend.title=element_blank()) +
  labs(x="Year of observation",
       y="Number of observations",
       title="Observations by Commonwealth conservation status (condensed) in Oberon LGA",
       subtitle = "NSW BioNet Atlas data as at 22 Feb 2026")
...
bionet_oberon_filtered_sf %>%
  filter(year(DateLast) > 1990, !red_list_category_condensed %in% c("Least Concern", "Nil")) %>%
  ggplot(aes(x=year(DateLast), fill=red_list_category_condensed)) +
           geom_bar(position = position_dodge2(preserve = "single")) +
  theme_minimal() +
  theme(legend.title=element_blank()) +
  labs(x="Year of observation",
       y="Number of observations",
       title="Observations by IUCN Red List status (condensed) in Oberon LGA",
       subtitle = "NSW BioNet Atlas data as at 22 Feb 2026")
...
bionet_oberon_filtered_sf %>%
  filter(year(DateLast) > 1990, !species_statuses %in% c(
        "Aust:Nil; NSW:Nil; IUCN:Nil",
        "Aust:Nil; NSW:Nil; IUCN:Least Concern", 
        "Aust:Nil; NSW:Protected; IUCN:Least Concern",
        "Aust:Nil; NSW:Protected; IUCN:Nil")) |>
  ggplot(aes(x=year(DateLast), fill=species_statuses)) +
           geom_bar(position = position_dodge2(preserve = "single")) +
  theme_minimal() +
  theme(legend.title=element_blank()) +
  labs(x="Year of observation",
       y="Number of observations",
       title="Observations by combined species status (condensed) in Oberon LGA",
       subtitle = "NSW BioNet Atlas data as at 22 Feb 2026")
...
bionet_oberon_filtered_sf %>%
  filter(year(DateLast) > 1990, !species_statuses %in% c(
        ...
        "Aust:Nil; NSW:Protected; IUCN:Least Concern",
        "Aust:Nil; NSW:Protected; IUCN:Nil")) |>
  gt(rowname_col = "ScientificName", groupname_col = "species_statuses")
...
ala_oberon_media_info <- request_metadata() |>
  filter(media == ala_oberon) |>
  compute() |>
  collect() |>
  left_join(ala_oberon_images)
